...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE products SET name = ?, sku = ?, barcode = ?, category = ?, purchase_rate = ?, selling_rate = ?, quantity = ?, aisle = ?, rack = ?, bin = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssddisssi", $name, $sku, $barcode, $category, $purchase_rate, $selling_rate, $quantity, $aisle, $rack, $bin, $id);
    $stmt->execute();
...
<form method="post">
    ...
    <div class="row">
        <div class="col-md-4">
            ...
            <input type="text" class="form-control" id="aisle" name="aisle" value="<?php echo $product['aisle']; ?>" required>
        </div>
        <div class="col-md-4">
            <label for="rack" class="form-label">Rack</label>
            <input type="text" class="form-control" id="rack" name="rack" value="<?php echo $product['rack']; ?>" required>
        </div>
        <div class="col-md-4">
            <label for="bin" class="form-label">Bin</label>
            <input type="text" class="form-control" id="bin" name="bin" value="<?php echo $product['bin']; ?>" required>
        </div>
    </div>
